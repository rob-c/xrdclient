...
from xrdclient.copy import tpc as engine_tpc
from xrdclient.errors import ChecksumMismatchError, UnsupportedError, kXR_Unsupported
from xrdclient.proto import constants as c
from xrdclient.testing import FakeDAVServer, FakeServer, error, frame
...
def test_download_writes_the_local_file(server, tmp_path):
    ...
    result = xrdclient.copy(server.url / "data/a.root", target)
...
def test_upload_writes_the_remote_file(src, server):
    result = xrdclient.copy(src, server.url / "up.bin")
...
def test_remote_to_remote_streams_through_this_process(server):
    with FakeServer() as other:
        xrdclient.copy(server.url / "data/a.root", other.url / "copied.root")
        assert other.contents("/copied.root") == b"hello world"


def test_local_to_local_needs_no_server(src, tmp_path):
    result = xrdclient.copy(src, tmp_path / "clone.bin")
...
def test_progress_reports_every_chunk_and_ends_at_the_size(src, server):
    ...
    xrdclient.copy(
        ...
        server.url / "p.bin",
        chunk_size=1024,
        config=xrdclient.Config(parallel_chunks=1),
...
def test_a_download_knows_the_total_up_front(server, tmp_path):
    totals = []
    xrdclient.copy(
        server.url / "data/a.root", tmp_path / "o", progress=lambda d, t: totals.append(t)
    )
    assert totals == [11]


def test_a_small_chunk_size_still_copies_every_byte(src, server):
    xrdclient.copy(src, server.url / "chunky.bin", chunk_size=7)
...
def test_overwrite_is_the_default_and_can_be_declined(src, tmp_path, server):
    ...
    xrdclient.copy(src, target)
    assert target.read_bytes() == PAYLOAD
    with pytest.raises(FileExistsError):
        xrdclient.copy(src, target, overwrite=False)
    with pytest.raises(FileExistsError):
        xrdclient.copy(src, server.url / "data/a.root", overwrite=False)


def test_a_missing_source_raises_the_usual_error(tmp_path, server):
    with pytest.raises(FileNotFoundError):
        xrdclient.copy(server.url / "nope.root", tmp_path / "x")
    with pytest.raises(FileNotFoundError):
        xrdclient.copy(tmp_path / "nope", server.url / "x")
...
def test_the_checksum_is_taken_against_the_target_when_it_is_remote(src, server):
    result = xrdclient.copy(src, server.url / "v.bin")
...
def test_a_corrupted_transfer_is_caught(src, server, monkeypatch):
    ...
    with pytest.raises(ChecksumMismatchError) as caught:
        xrdclient.copy(src, server.url / "bad.bin")
    assert caught.value.expected == "deadbeef"


def test_verification_can_be_declined(src, server, monkeypatch):
    monkeypatch.setattr("xrdclient.testing.server._checksum", lambda algorithm, data: "deadbeef")
    assert xrdclient.copy(src, server.url / "unchecked.bin", verify=False).checksum is None
...
def test_a_server_that_cannot_checksum_degrades_quietly(src, server, monkeypatch):
    ...
    assert xrdclient.copy(src, server.url / "q.bin").checksum is None
    with pytest.raises(UnsupportedError):
        xrdclient.copy(src, server.url / "q.bin", verify=True)


def test_the_algorithm_is_configurable(src, server):
    result = xrdclient.copy(src, server.url / "md5.bin", algorithm="md5")
    assert result.checksum.algorithm == "md5"


def test_a_download_verifies_against_the_source(server, tmp_path):
    result = xrdclient.copy(server.url / "data/a.root", tmp_path / "d.root")
    assert result.checksum.value == xrdclient.crypto.checksum_bytes("adler32", b"hello world")
...
def test_the_result_describes_the_transfer(server, tmp_path):
    result = xrdclient.copy(server.url / "data/a.root", tmp_path / "r.root")
    assert result.size == 11
...
def test_an_instantaneous_copy_has_an_infinite_rate():
    assert xrdclient.CopyResult("a", "b", 10, 0.0).rate == float("inf")
...
def test_copy_tree_uploads_a_whole_directory(tmp_path, server):
    ...
    results = xrdclient.copy_tree(tmp_path, server.url / "tree")
...
def test_copy_tree_downloads_and_creates_local_directories(server, tmp_path):
    ...
    xrdclient.copy_tree(server.url / "data", tmp_path / "down")
...
def test_copy_tree_passes_options_through(tmp_path, server):
    ...
    results = xrdclient.copy_tree(tmp_path, server.url / "opts", chunk_size=8, verify=False)
...
def test_exclude_skips_what_matches_it(tree, server):
    xrdclient.copy_tree(tree, server.url / "sel", exclude=["*.log"])
    assert copied(server, "/sel") == ["one.root", "sub/two.root"]


def test_include_is_a_whitelist_and_exclude_beats_it(tree, server):
    xrdclient.copy_tree(tree, server.url / "inc", include=["*.root", "*.log"], exclude=["sub/*"])
...
def test_what_no_include_pattern_names_stays_behind(tree, server):
    ...
    assert xrdclient.copy_tree(tree, server.url / "none", include=["*.parquet"]) == []
    assert copied(server, "/none") == []


def test_a_dry_run_reports_the_transfers_it_did_not_make(tree, server):
    results = xrdclient.copy_tree(tree, server.url / "dry", dry_run=True)
...
def test_sync_by_size_skips_what_is_already_there(tree, server):
    first = xrdclient.copy_tree(tree, server.url / "syn", sync="size")
    again = xrdclient.copy_tree(tree, server.url / "syn", sync="size")
    assert len(first) == 3 and again == []
    (tree / "one.root").write_bytes(b"longer than before")
    assert [r.size for r in xrdclient.copy_tree(tree, server.url / "syn", sync="size")] == [18]


def test_sync_by_checksum_notices_a_change_of_the_same_length(tree, server):
    xrdclient.copy_tree(tree, server.url / "cks", sync="checksum")
    assert xrdclient.copy_tree(tree, server.url / "cks", sync="checksum") == []
    (tree / "one.root").write_bytes(b"ONE")
    assert len(xrdclient.copy_tree(tree, server.url / "cks", sync="checksum")) == 1
...
def test_sync_by_mtime_copies_again_when_the_source_is_newer(tree, tmp_path_factory):
    ...
    assert len(xrdclient.copy_tree(tree, mirror, sync="mtime")) == 3
    for path in mirror.rglob("*.*"):
        os.utime(path, (2_000_000_000, 2_000_000_000))
    assert xrdclient.copy_tree(tree, mirror, sync="mtime") == []
    os.utime(tree / "one.root", (2_100_000_000, 2_100_000_000))
    assert len(xrdclient.copy_tree(tree, mirror, sync="mtime")) == 1


def test_sync_copies_a_file_the_target_does_not_have_at_all(tree, server):
    assert len(xrdclient.copy_tree(tree, server.url / "fresh", sync="size")) == 3


def test_delete_removes_what_the_source_no_longer_has(tree, server):
    xrdclient.copy_tree(tree, server.url / "del")
    (tree / "notes.log").unlink()
    xrdclient.copy_tree(tree, server.url / "del", delete=True)
    assert copied(server, "/del") == ["one.root", "sub/two.root"]


def test_a_dry_run_deletes_nothing(tree, server):
    xrdclient.copy_tree(tree, server.url / "dd")
    (tree / "notes.log").unlink()
    xrdclient.copy_tree(tree, server.url / "dd", delete=True, dry_run=True)
...
def test_delete_takes_an_exclusion_at_its_word(tree, server):
    ...
    xrdclient.copy_tree(tree, server.url / "sel2")
    xrdclient.copy_tree(tree, server.url / "sel2", exclude=["*.log"], delete=True)
    assert copied(server, "/sel2") == ["one.root", "sub/two.root"]


def test_remove_source_makes_the_copy_a_move(src, server):
    result = xrdclient.copy(src, server.url / "moved.bin", remove_source=True)
...
def test_a_move_that_fails_verification_keeps_the_original(src, server):
    ...
    with pytest.raises(ChecksumMismatchError):
        xrdclient.copy(src, server.url / "kept.bin", verify=True, remove_source=True)
    assert src.exists()


def test_a_remote_source_is_removed_too(server, tmp_path):
    xrdclient.copy(server.url / "data/a.root", tmp_path / "here.root", remove_source=True)
...
def test_a_dry_run_of_a_stream_has_nothing_to_measure(server):
    result = xrdclient.copy(io.BytesIO(b"data"), server.url / "never.bin", dry_run=True)
...
def test_a_probe_of_something_that_is_not_there_is_not_a_size(server, tmp_path):
    ...
    assert engine._probe(xrdclient.parse(str(tmp_path / "absent")), cfg) is None
...
def test_a_resumed_download_keeps_what_is_already_there(server, tmp_path):
    ...
    result = xrdclient.copy(server.url / "data/a.root", target, resume=True)
    assert target.read_bytes() == b"hello world"
...
def test_a_resumed_upload_keeps_what_the_server_already_has(src, server):
    with FakeServer(files={"/part.bin": PAYLOAD[:4096]}) as dst:
        result = xrdclient.copy(src, dst.url / "part.bin", resume=True)
...
def test_resume_copies_the_whole_file_when_there_is_nothing_to_carry_on_from(server, tmp_path):
    ...
    result = xrdclient.copy(server.url / "data/a.root", tmp_path / "fresh.root", resume=True)
...
def test_an_empty_target_is_not_a_partial_one(server, tmp_path):
    ...
    assert xrdclient.copy(server.url / "data/a.root", target, resume=True).resumed_at == 0
    assert target.read_bytes() == b"hello world"
...
def test_a_target_longer_than_its_source_is_not_a_partial_copy(server, tmp_path):
    ...
    with pytest.raises(ValueError, match="not a partial copy"):
        xrdclient.copy(server.url / "data/a.root", target, resume=True)
...
def test_resume_and_an_exclusive_target_contradict_each_other(server, tmp_path):
    with pytest.raises(ValueError, match="overwrite=False"):
        xrdclient.copy(server.url / "data/a.root", tmp_path / "x", resume=True, overwrite=False)


def test_resume_needs_a_url_on_both_sides(server):
    with pytest.raises(ValueError, match="already-open stream"):
        xrdclient.copy(server.url / "data/a.root", io.BytesIO(), resume=True)
...
def test_an_http_target_cannot_be_resumed(tmp_path):
    ...
    with FakeDAVServer(files={"/d/part.bin": PAYLOAD[:512]}) as dav:
        with pytest.raises(UnsupportedError, match="cannot resume a copy into http"):
            xrdclient.copy(src_file, dav.url / "d/part.bin", resume=True)
...
def test_a_resumed_copy_can_skip_verification(server, tmp_path):
    ...
    target.write_bytes(b"HELLO ")
    result = xrdclient.copy(server.url / "data/a.root", target, resume=True, verify=False)
...
def test_a_resumed_copy_degrades_when_an_end_cannot_checksum(server, tmp_path, monkeypatch):
    ...
    target.write_bytes(b"hello ")
    assert xrdclient.copy(server.url / "data/a.root", target, resume=True).checksum is None
    target.write_bytes(b"hello ")
    with pytest.raises(UnsupportedError):
        xrdclient.copy(server.url / "data/a.root", target, resume=True, verify=True)
...
def test_a_resumed_tree_carries_on_file_by_file(tmp_path, server):
    ...
    with FakeServer(files={"/t/a.bin": PAYLOAD[:2048]}, dirs=["/t"]) as dst:
        results = xrdclient.copy_tree(tmp_path, dst.url / "t", resume=True)
...
def test_several_files_are_copied_at_once(tree, server, monkeypatch):
    ...
    results = xrdclient.copy_tree(tree, server.url / "wide", workers=3)
...
def test_the_config_says_how_many_files_when_the_call_does_not(tree, server, monkeypatch):
    ...
    monkeypatch.setattr(engine, "copy", barred)
    xrdclient.copy_tree(tree, server.url / "cfg", config=xrdclient.Config(parallel_files=3))
...
def test_workers_do_not_reorder_the_results(tree, server):
    ...
    ordered = xrdclient.copy_tree(tree, server.url / "a", workers=1)
    concurrent = xrdclient.copy_tree(tree, server.url / "b", workers=4)
...
def test_progress_over_a_tree_of_workers_counts_the_whole_tree(tree, server):
    seen = []
    xrdclient.copy_tree(
        tree, server.url / "tp", workers=3, progress=lambda d, t: seen.append((d, t))
    )
...
def test_one_worker_reports_progress_a_file_at_a_time(tree, server):
    ...
    seen = []
    xrdclient.copy_tree(
        tree, server.url / "sp", workers=1, progress=lambda d, t: seen.append((d, t))
    )
...
def test_the_first_failure_stops_the_tree_rather_than_copying_on(tmp_path, server, monkeypatch):
    ...
    # whatever the walk finds first
    doomed = next(engine._walk(parse(tmp_path), xrdclient.Config()))
    attempted = []
    ...
    with pytest.raises(OSError, match="not going anywhere"):
        xrdclient.copy_tree(tmp_path, server.url / "stop", workers=2)
    assert len(attempted) < 20


def test_workers_skip_what_sync_says_is_already_there(tree, server):
    assert len(xrdclient.copy_tree(tree, server.url / "ws", workers=3, sync="size")) == 3
    assert xrdclient.copy_tree(tree, server.url / "ws", workers=3, sync="size") == []
...
def test_a_parallel_download_reassembles_in_order(server, tmp_path):
    with FakeServer(files={"/big.bin": PAYLOAD}) as srv:
        result = xrdclient.copy(srv.url / "big.bin", tmp_path / "big.bin", chunk_size=512)
...
def test_a_file_too_short_to_share_out_stays_in_one_stream(src, server):
    ...
    plan = engine._spread(parse(src), server.url / "x.bin", xrdclient.Config(), len(PAYLOAD))
    assert plan is None
...
def test_an_http_target_is_never_divided(tmp_path):
    ...
    with FakeDAVServer(dirs=["/d"]) as dav:
        assert engine._spread(parse(source), dav.url / "d/x.bin", xrdclient.Config(), 1024) is None
        xrdclient.copy(source, dav.url / "d/x.bin", chunk_size=1024)
...
def test_a_resumed_transfer_is_not_also_a_divided_one(server, tmp_path):
    ...
    with FakeServer(files={"/big.bin": PAYLOAD}) as srv:
        ...
        result = xrdclient.copy(srv.url / "big.bin", target, chunk_size=512, resume=True)
    assert target.read_bytes() == PAYLOAD
    assert (result.resumed_at, result.size) == (4096, len(PAYLOAD) - 4096)
...
def test_a_scheme_the_library_does_not_speak_is_reported_as_unsupported(server, tmp_path):
    ...
    with pytest.raises(UnsupportedError, match="cannot read from ftp"):
        xrdclient.copy("ftp://example.org/f.root", os.fspath(tmp_path / "out"))
    with pytest.raises(UnsupportedError, match="cannot write to ftp"):
        xrdclient.copy(server.url / "data/a.root", "ftp://example.org/f.root")
...
def test_a_source_that_only_knows_how_to_read_is_still_copied(server):
    ...
    xrdclient.copy(ReadOnly(b"minimal" * 100), server.url / "minimal.bin")
...
def test_a_source_of_unknown_size_leaves_the_allocation_hint_out():
    ...
    opaque = engine_tpc._dst_opaque("k" * 32, xrdclient.parse("root://s//f.root"), "s:1094", -1, "")
...
def test_a_copy_through_the_window_is_byte_for_byte(tmp_path, depth):
    ...
    result = xrdclient.copy(
        source, tmp_path / "out.bin", config=xrdclient.Config(in_flight=depth), chunk_size=4096
    )
...
def test_progress_and_digests_still_see_the_file_in_order():
    ...
    digest = xrdclient.crypto.new("adler32")
    steps: list[int] = []
    ...
    assert digest.hexdigest() == xrdclient.crypto.checksum_bytes("adler32", payload)
